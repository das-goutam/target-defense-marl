...
class TargetDefenseRLTrainer:
    ...
    def train_episode(self) -> Tuple[float, float, float]:
        ...
        sensing_rate = 0
        if hasattr(self.env.scenario, 'attacker_sensed'):
            total_sensed = self.env.scenario.attacker_sensed.float().sum().item()
            total_attackers = self.config.num_attackers * self.config.num_envs
            sensing_rate = total_sensed / total_attackers if total_attackers > 0 else 0
    ...
    def print_final_statistics(self):
        ...
        # Performance assessment based on complete interceptions
        final_interception = np.mean(self.interception_rates[-window:])
        
        print("\nPerformance Assessment (Complete Interceptions):")
        if final_interception > 0.8:
            print("  🏆 EXCELLENT: Complete interception in >80% of episodes!")
        elif final_interception > 0.6:
            print("  ✅ GOOD: Complete interception in >60% of episodes")
        elif final_interception > 0.4:
            print("  ⚠️  MODERATE: Complete interception in >40% of episodes")
        else:
            print("  ❌ NEEDS IMPROVEMENT: Complete interception in <40% of episodes")
    ...
    def evaluate(self, num_episodes: int = 10) -> Dict[str, float]:
        ...
        for _ in range(num_episodes):
            ...
            # Track complete interceptions
            if hasattr(self.env.scenario, 'attacker_sensed'):
                # Complete interceptions
                all_sensed_per_env = self.env.scenario.attacker_sensed.all(dim=1)
                complete_interceptions = all_sensed_per_env.float().sum().item()
                eval_interceptions.append(complete_interceptions / self.config.num_envs)
                
                # Average sensing
                total_sensed = self.env.scenario.attacker_sensed.float().sum().item()
                total_attackers = self.config.num_attackers * self.config.num_envs
                eval_sensing.append(total_sensed / total_attackers if total_attackers > 0 else 0)
        
        results = {
            'avg_reward': np.mean(eval_rewards),
            'interception_rate': np.mean(eval_interceptions) if eval_interceptions else 0,
            'sensing_rate': np.mean(eval_sensing) if eval_sensing else 0
        }
        
        print(f"Evaluation Results ({num_episodes} episodes):")
        print(f"  Average Reward: {results['avg_reward']:.3f}")
        print(f"  Complete Interception Rate: {results['interception_rate']:.2%}")
        print(f"  Average Sensing Rate: {results['sensing_rate']:.2%}")
